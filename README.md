...
| TRUST_PROXY                | true                                 | ```true``` \| ```false``` \| ```number``` \| ```string``` Express trust proxy setting. Can be a boolean (true/false), a positive number representing the nth hop from the front-facing proxy, or a string with IP addresses/subnets/predefined names (e.g., 'loopback', '192.168.1.1'). See https://expressjs.com/en/guide/behind-proxies.html for more details. |
